...
//Rare exceptions to indicate a bug in the _chapter05_tigram code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_chapter05_tigram = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_chapter05_tigram]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.chapter05_tigram.module"
