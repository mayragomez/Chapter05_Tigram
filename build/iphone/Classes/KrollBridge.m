...
NSString * Chapter05_Tigram$ModuleRequireFormat = @"(function(exports){"
...
@implementation Chapter05_TigramObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_chapter05_tigram);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Chapter05_Tigram global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Chapter05_Tigram" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_chapter05_tigram = [[Chapter05_TigramObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_chapter05_tigram];
	
	NSString *_chapter05_tigramNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _chapter05_tigramNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_chapter05_tigram valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Chapter05_Tigram$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.chapter05_tigram.kroll" 
